...
- 日付はカレンダー、時刻は時・分を直接スクロールするホイールから設定できます。
...
- Androidの日時通知は端末内で処理し、ロック画面の内容は初期状態で非表示です。設定で表示を有効にした場合だけタイトルを表示します。
- 通知から「完了」「後で行う」をアプリを開かずに実行でき、成功通知には対象操作だけを10秒以内に戻す「元に戻す」を表示します。
- 通知は通常のActivityを勝手に前面表示しません。省電力状態（Doze）ではOSの都合で通知が遅れる場合があります。
